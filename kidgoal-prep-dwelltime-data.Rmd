...
#Q: does age differ across the three conditions?
age <- dt_data %>% 
...
age %>% 
  group_by(condition) %>% 
  summarise(mean_age = mean(age_months, na.rm = TRUE),
            mean_years = mean_age/12)

summary(aov(age_months ~ condition, data = age))
